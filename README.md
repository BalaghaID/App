[![BalaghaID](static/logo.png "BalaghaID")](https://Balagha-ID.app)
# [BalaghaID App (v0.1.0)](https://github.com/BalaghaID/App/tree/v0.1.0)
### Part of the [BALAGHA Score Project](https://project.balaghascore.com)

An interactive application for identifying Arabic rhetorical devices through guided decision-making.


&nbsp;
## ℹ️ About
The **BalaghaID App** uses a structured decision tree based on the [Arabic Rhetorical Device Taxonomy (ARDT v0.1.1)](https://github.com/Al-Balagha/Arabic-Rhetoric/tree/v0.1.1), guiding users step by step through diagnostic questions that progressively narrow down the rhetorical phenomenon under analysis.

Rather than relying on prior terminological knowledge, BalaghaID focuses on observable linguistic features and functional cues, allowing users to arrive at a principled identification of rhetorical devices.

BalaghaID is configured to guide users toward principled identification of the [94 rhetorical devices](https://balagha-id.app/list) defined in the ARDT, spanning the classical domains of [ʿilm al-maʿānī](https://al-balagha.com/wiki/Encyclopedia_of_Arabic_Rhetoric#A), [ʿilm al-bayān](https://al-balagha.com/wiki/Encyclopedia_of_Arabic_Rhetoric#B), and [ʿilm al-badīʿ](https://al-balagha.com/wiki/Encyclopedia_of_Arabic_Rhetoric#C), as well as recognised [negative rhetorical effects](https://al-balagha.com/wiki/Encyclopedia_of_Arabic_Rhetoric#D).

📄 Zenodo DOI: [https://github.com/BalaghaID/App/tree/v0.1.0](https://github.com/BalaghaID/App/tree/v0.1.0)


&nbsp;
### 🎯 Purpose
BalaghaID is a decision-support tool intended to assist human annotators in systematically applying the Arabic Rhetorical Device Taxonomy. It does not perform automatic rhetorical detection. 

In research contexts, BalaghaID may be used during manual annotation to reduce oversight, improve consistency, and operationalise complex taxonomic distinctions.

BalaghaID is designed for researchers, students, and practitioners of Arabic rhetoric, digital humanities, and related fields, and serves both as an analytical tool and as a pedagogical aid.

BalaghaID forms part of the broader [BALAGHAScore framework](https://project.balaghascore.com/), which aims to operationalise [Arabic rhetoric](https://al-balagha.com/) for systematic analysis and comparative study.

Users are invited to explore [BalaghaID](https://Balagha-ID.app), apply it to their own texts, and incorporate it into research, teaching, or exploratory rhetorical analysis. Feedback, methodological discussion, and critical engagement are welcome as the tool continues to develop.


&nbsp;
### 🔄 How to use
**Option 1 (Recommended):** Download the self-contained [BalaghaID_v0.1.0.html](https://github.com/BalaghaID/App/releases/download/v0.1.0/BalaghaID_v0.1.0.html) file and open it on any modern web browser. **No installation or internet connection is required.** Follow the on-screen instructions to guide you through Arabic rhetorical device identification.

**Option 2:** Visit the [Downloads page](https://balagha-id.app/download) for BalaghaID to download the latest version of the app.

**Option 3:** Visit [BalaghaID.app](https://balagha-id.app) to use the tool directly online.


&nbsp;
### 📦 Repository Contents
```
/
├── README.md
├── LICENSE.txt
├── CITATION.cff
├── CHANGELOG.md
├── BalaghaID_v0.1.0.html
└── static/
```


&nbsp;
### 🔄 Versioning
The BalaghaID App uses **Semantic Versioning (SemVer)**:
* [15 December 2025 - v0.1.0](https://github.com/BalaghaID/App/tree/v0.1.0)  
First public release of the BalaghaID App on GitHub and Zenodo.


&nbsp;
### 📖 How to Cite
**APA:**
Marathe, M. (2025). BalaghaID.app: Arabic Rhetorical Device Identification with an Interactive App (v0.1.0) [Software]. Zenodo. https://github.com/BalaghaID/App/tree/v0.1.0

**Chicago:**
Marathe, Mandar. 'BalaghaID.app: Arabic Rhetorical Device Identification with an Interactive App (v0.1.0)'. Software, Zenodo, 2025. https://github.com/BalaghaID/App/tree/v0.1.0

**BibTeX:**
```bibtex
@software{BalaghaID2025,
  author = {{Mandar Marathe}},
  title = {BalaghaID.app: Arabic Rhetorical Device Identification with an Interactive App},
  year = {2025},
  version = {0.1.0},
  url = {https://github.com/BalaghaID/App/tree/v0.1.0}
```


&nbsp;<br>  
---  
## 📬 About the Author
**[Dr Mandar Marathe](https://marathe.org)** | [SOAS Profile](https://www.soas.ac.uk/about/mandar-marathe)  
PhD Researcher in Arabic Rhetoric | SOAS University of London  
[![Project Home](https://img.shields.io/badge/BalaghaID%20App-Home-green)](https://Balagha-ID.app)  

### Related Projects
- 📚 [Encyclopedia of Arabic Rhetoric](https://al-balagha.com) - Reference source for Arabic rhetorical devices  
- 💯 [BALAGHA Score](https://balaghascore.com) - Arabic rhetorical density analysis  
- 📖 [Balagha Corpus](https://balagha-corpus.com) - Annotated texts for Arabic rhetorical analysis  
- 📡 [BalaghaBase.org](https://balaghabase.org) - Semantic knowledge graph for Arabic rhetoric  
- 🔬 [Rhetorical Density](https://rhetorical-density.com) - Methodology and discussion of rhetorical density as a quantitative metric  

### Connect
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profile-blue)](https://www.linkedin.com/in/mandar-marathe-uk/) [![ORCID](https://img.shields.io/badge/ORCID-Profile-green.svg)](https://orcid.org/0000-0002-6927-6836) [![Google Scholar](https://img.shields.io/badge/Google%20Scholar-Profile-blue)](https://scholar.google.com/citations?user=w-bT-iYAAAAJ)  
---
_Last updated: 15 December 2025._  